...
([`docs/kpi/metrics-store.md`](docs/kpi/metrics-store.md)), then **dual-written
to Datadog** since the account went Pro (the addendum in the same doc:
Postgres stays the record, Datadog is the public picture). Nothing here calls
a model: the measures are ordinary Python, which is where "deterministic
numbers, LLM narrative" stops being a principle and becomes a call graph.
...
### The live Datadog surface (RC1-305, RC1-329)

Everything below is generated by `kpi/datadog.py` from the same adopted trees
and instrument reports as the Grafana set, and upserted by title/name so the
links survive regeneration. Owner login required — a public share URL for any
dashboard can be minted (and revoked) through the API on demand.
...
number's working, in the reading's own words, next to the chart.

```bash
python -m kpi.datadog dashboards --push    # create/update both dashboards
python -m kpi.datadog monitors --push      # create/update the six monitors
```
