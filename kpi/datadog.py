...
def fleet_slo_payload() -> dict:
    """One metric-based SLO over the whole agent fleet: share of production
    runs — one LLM Observability trace per review, digest or narration —
    that completed without error (RC1-407).

    The population is `ml_obs.trace`, which every ml_app reports, including
    the n8n concert workflow that posts hand-built spans to the intake and
    never reaches APM (the RC1-324 query read `trace.anthropic.request`, so
    it covered whichever *services* ran ddtrace, not the fleet). Corpus
    sweeps and `measure_pr.py` runs report under the same ml_app as
    production (`service:evals`, `service:dry-run`; RC1-411) and are
    excluded: they would dilute the ratio. The unit is the trace, not the
    span, so a review is one run however many stages it has. Errors come
    from `ml_obs.trace.error`, the metric the intake derives from a root
    span's `status`, rather than the `error` tag, which only SDK-built
    spans carry. `default_zero` because `.error` only materialises after
    the first error — a fleet that has never failed must read 100 %, not
    no-data."""
    production = "!service:evals,!service:dry-run"
    hits = f"sum:ml_obs.trace{{{production}}}.as_count()"
    errors = f"default_zero(sum:ml_obs.trace.error{{{production}}}.as_count())"
    return {
        "name": FLEET_SLO_NAME,
        "type": "metric",
        ...
        "description": (
            "Share of the agent fleet's production runs (one LLM Observability "
            "trace each) that completed without error, across every ml_app; "
            "corpus sweeps and dry runs (service:evals, service:dry-run) "
            "excluded. Generated by kpi/datadog.py (RC1-324, widened from APM "
            "to LLM Obs traces in RC1-407)."
        ),
    ...
    if args.cmd == "slos":
        if args.push:
            ...
            print(
                ...
                "production runs without error, fleet-wide"
            )
